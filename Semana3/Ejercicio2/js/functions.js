...
function dniLetter( dni ) {
	...
	if (isNaN( dni ) || dni < 0 ) { return 'Invalid parameter'; }

	var lockup = ['T', 'R', 'W', 'A', 'G', 'M', 'Y', 'F', 'P', 'D', 'X', 'B', 'N', 'J', 'Z', 'S', 'Q', 'V', 'H', 'L', 'C', 'K', 'E'];
	var result = '';

	for( var i = 0; i < 23; i++ ) { result = lockup[dni % 23];}
...
function findLongestWord( array_words ){ 
	...
	for (var i = 0; i < array_words.length; i++ ) { 
		...
		if (array_words[i].length > longitud ) { 

			longitud = array_words[i].length;
		}
...
array_lengths.push( findLongestWord( ["Refrescante", "Esternocleidomastoideo", "Ojo"] ) );
...
var sum = 0;
for (var i = 0, longitud = array.length; i < longitud; i += 1 ) {
	sum += array_lengths[i];
};
return sum / longitud; 
